...
class Parser {
	...
	walk(g,id,nodes) {
		...
		}, NewExpression : function(node,state,cont) {
	...
	walkDeclarations(body,isRoot) {
		...
		while(_g < body.length) {
			...
			case "VariableDeclaration":
				...
				break;
			default:
				haxe_Log.trace("WARNING: Unexpected " + node.type + ", at character " + node.start,{ fileName : "tool/src/Parser.hx", lineNumber : 182, className : "Parser", methodName : "walkDeclarations"});
			}
		}
	}
	inspectBlockStatement(def) {
		if(this.collectTypesInNode(def).length > 0) {
	...
	collectTypesInNode(blockNode) {
		let foundTypes = [];
		let _gthis = this;
		let visitors = { NewExpression : function(node) {
			if(node.callee != null && node.callee.type == "Identifier") {
				let name = node.callee.name;
				if(Object.prototype.hasOwnProperty.call(_gthis.types,name) && foundTypes.indexOf(name) < 0) {
					foundTypes.push(name);
				}
			}
		}};
		ast_Walk.recursive(blockNode,{ },visitors);
		return foundTypes;
	}
	inspectIfStatement(test,def) {
		...
		if(test.type == "BinaryExpression") {
			...
			if(path.length > 1 && path[1] == "prototype") {
				this.tag(path[0],def);
			} else if(path.length > 0 && Object.prototype.hasOwnProperty.call(this.types,path[0])) {
				this.tag(path[0],def);
			}
			...
			if(decl.id != null) {
				...
				if(decl.init != null) {
					...
					case "ArrayExpression":
						if(this.collectTypesInNode(init).length > 0) {
							def.__tag__ = "__reserved__";
						}
						break;
					case "AssignmentExpression":
